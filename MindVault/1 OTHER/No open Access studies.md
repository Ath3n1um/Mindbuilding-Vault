all studies that are closed source

A controlled trial of
working memory training for children and adolescents with ADHD

 Long-term far-transfer effects of working memory
training in children with ADHD: a randomized controlled trial

Working memory training for patients with acquired brain injury:
effects in daily life

Computerized working memory training-a method of cognitive rehabilitation after stroke

3D Multiple Object Tracking Boosts Working Memory Span: Implications for Cognitive Training in Military Populations

Randomized-controlled trial examining differing content of ABA therapy delivered to autistic children

Randomized controlled trial evaluation of ABA content on IQ gains in children with autism

Teaching foundational perspective-taking skills to children with autism using the PEAK-T curriculum: Single-reversal “I-You” deictic frames

The use of in-situ behavioral skills training to improve staff implementation of the PEAK relational training system

The relationship between derived mutually entailed relations and the function of challenging behavior in children with autism: Comparing the PEAK-E-PA and the QABF

Abstraction of tactile properties by individuals with autism and down syndrome using a picture‐based communication system

Teaching metaphorical extensions of private events to children with emotional disorders using PEAK-direct training

Derived coordinated symmetrical and transitive gustatory-visual-auditory relations in children with disabilities

Establishing derived categorical responding in children with disabilities using the PEAK-E curriculum

Teaching complex verbal operants to children with autism and establishing generalization using the peak curriculum

A demonstration of higher‐order response class development in children

Assessing the Relationship between Intelligence and the PEAK Relational Training System

Equivalence‐based instruction of academic skills: Application to adolescents with autism

Student outcomes after 1 year of front line staff implementation of the PEAK curriculum

Quantifying differential within-class stimulus relatedness using

Analogical relations. In T. Mulhern, Relational Frame Theory Made Simple ???

Assessing and teaching complex relational operants: Analogy and hierarchy/Applied Behavior Analysis of Language and Cognition

Relational frame theory: A post-Skinnerian account of human language and cognition

Derived Relational Responding Applications for Learners with Autism and Other Developmental Disabilities: A Progressive Guide to Change.

Nurturing genius: Using relational frame theory to address a foundational aim of psychology

The effect of SMART Relational Skills training on Intelligence Quotients: Controlling for Individual Differences in Attentional Skills and Baseline IQ.

Relational Training Intervention as add-on therapy to current specific treatments in patients with mild-to-moderate Alzheimer’s disease

Comparing the effects of derived relational training and computer coding on intellectual potential in school-age children

